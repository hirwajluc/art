            ...
            <div class="settings-grid">
                ...
                <div class="settings-card full">
                    <div class="card-header" style="background:#7c3aed;">
                        <h3><i class="fas fa-envelope-open-text"></i> Email / SMTP Settings</h3>
                    </div>
                    <div class="card-content">
                        <?php
                            $smtp = [
                                'host'       => $competitionSettings['smtp_host']       ?? '',
                                'port'       => $competitionSettings['smtp_port']       ?? '587',
                                'username'   => $competitionSettings['smtp_username']   ?? '',
                                'password'   => $competitionSettings['smtp_password']   ?? '',
                                'encryption' => $competitionSettings['smtp_encryption'] ?? 'tls',
                                'from_email' => $competitionSettings['smtp_from_email'] ?? 'info@greaterproject.eu',
                                'from_name'  => $competitionSettings['smtp_from_name']  ?? 'GREATER Art Competition',
                            ];
                        ?>

                        <?php if (!empty($competitionSettings['smtp_host'])): ?>
                        <div class="alert alert-success" style="margin-bottom:18px;">
                            <i class="fas fa-check-circle"></i>
                            SMTP configured: <strong><?php echo htmlspecialchars($smtp['host']); ?>:<?php echo htmlspecialchars($smtp['port']); ?></strong>
                            &nbsp;&nbsp;
                            <a href="?page=send_test_email" class="btn btn-sm" style="background:#065f46;color:#fff;padding:4px 14px;border-radius:6px;text-decoration:none;font-size:13px;display:inline-block;">
                        ...
                        <div class="alert alert-error" style="margin-bottom:18px;">
                            ...
                            No SMTP host configured — email sending is disabled. Fill in the form below.
                        </div>
                        <?php endif; ?>
                        ...
                        <form method="POST" action="?page=save_settings">
                            <div style="display:grid; grid-template-columns:1fr 1fr; gap:20px;">
                                <div class="form-group">
                                    <label class="form-label" style="font-weight:600;color:#374151;display:block;margin-bottom:6px;">SMTP Host</label>
                                    <input type="text" name="smtp_host" class="form-input"
                                           value="<?php echo htmlspecialchars($smtp['host']); ?>"
                                           placeholder="mail.greaterproject.eu"
                                           style="width:100%;padding:10px 14px;border:1.5px solid #d1d5db;border-radius:8px;font-size:14px;">
                                </div>
                                <div class="form-group">
                                    <label class="form-label" style="font-weight:600;color:#374151;display:block;margin-bottom:6px;">SMTP Port</label>
                                    <input type="number" name="smtp_port" class="form-input"
                                           value="<?php echo htmlspecialchars($smtp['port']); ?>"
                                           placeholder="587"
                                           style="width:100%;padding:10px 14px;border:1.5px solid #d1d5db;border-radius:8px;font-size:14px;">
                                </div>
                                <div class="form-group">
                                    <label class="form-label" style="font-weight:600;color:#374151;display:block;margin-bottom:6px;">SMTP Username</label>
                                    <input type="text" name="smtp_username" class="form-input"
                                           value="<?php echo htmlspecialchars($smtp['username']); ?>"
                                           placeholder="info@greaterproject.eu"
                                           style="width:100%;padding:10px 14px;border:1.5px solid #d1d5db;border-radius:8px;font-size:14px;">
                                </div>
                                <div class="form-group">
                                    <label class="form-label" style="font-weight:600;color:#374151;display:block;margin-bottom:6px;">SMTP Password</label>
                                ...
                                <div class="form-group">
                                    <label class="form-label" style="font-weight:600;color:#374151;display:block;margin-bottom:6px;">Encryption</label>
                                    <select name="smtp_encryption" style="width:100%;padding:10px 14px;border:1.5px solid #d1d5db;border-radius:8px;font-size:14px;">
                                        <option value="tls"  <?php echo $smtp['encryption']==='tls'  ? 'selected' : ''; ?>>STARTTLS (port 587 — recommended)</option>
                                        <option value="ssl"  <?php echo $smtp['encryption']==='ssl'  ? 'selected' : ''; ?>>SSL/TLS  (port 465)</option>
                                        <option value=""     <?php echo $smtp['encryption']===''     ? 'selected' : ''; ?>>None     (port 25 — not recommended)</option>
                                    </select>
                                </div>
                                <div style="display:grid; grid-template-columns:1fr 1fr; gap:12px; grid-column:1/-1;">
                                    <div class="form-group">
                                        <label class="form-label" style="font-weight:600;color:#374151;display:block;margin-bottom:6px;">From Name</label>
                                        <input type="text" name="smtp_from_name" class="form-input"
                                               value="<?php echo htmlspecialchars($smtp['from_name']); ?>"
                                               style="width:100%;padding:10px 14px;border:1.5px solid #d1d5db;border-radius:8px;font-size:14px;">
                                    </div>
                                    <div class="form-group">
                                        <label class="form-label" style="font-weight:600;color:#374151;display:block;margin-bottom:6px;">From Email</label>
                            ...
                            <div style="margin-top:14px;">
                                <p style="font-size:13px;color:#6b7280;margin-bottom:14px;">
                                    <i class="fas fa-info-circle"></i>
                                    Contact your hosting provider for SMTP credentials. Typically: host = <code>mail.yourdomain.com</code>, port 587, STARTTLS, username = your email address.
                                </p>
                                <button type="submit" class="btn" style="background:#7c3aed;color:#fff;padding:12px 32px;font-size:15px;">
